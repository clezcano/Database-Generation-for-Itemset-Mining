...
class InputFile:
    ...
    def getTransAvgSize(self):
        with open(self.filename, 'r') as f:
            transLength = [len(line.strip().split(self.delimeter)) for line in f.readlines()]
        return sum(transLength) / len(transLength)

    def getTransMaxSize(self):
        with open(self.filename, 'r') as f:
            transLength = [len(line.strip().split(self.delimeter)) for line in f.readlines()]
        return max(transLength)
